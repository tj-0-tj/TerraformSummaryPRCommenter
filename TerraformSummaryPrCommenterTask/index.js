...
exports.main = exports.generateStageSection = exports.calculateOverallStageStatus = exports.generateJobSummary = exports.summarizeTerraformPlan = exports.readTerraformPlan = void 0;
const axios_1 = __importDefault(require("axios"));
const fs = __importStar(require("fs"));
const _ = __importStar(require("lodash"));
const tl = require("azure-pipelines-task-lib/task");
// Reading inputs from ADO task
const TERRAFORM_PLAN_PATH = tl.getInput('TERRAFORM_PLAN_PATH', false) || './terraform-plan.json';
const ADO_STAGE_NAME = _.escapeRegExp(tl.getInput('ADO_STAGE_NAME', true) || '');
const ADO_JOB_LINK = tl.getInput('ADO_JOB_LINK', true) || '#';
const ADO_STAGE_LINK = tl.getInput('ADO_STAGE_LINK', true) || '#';
const GITHUB_TOKEN = tl.getInput('GITHUB_TOKEN', true) || '';
const GITHUB_OWNER = tl.getInput('GITHUB_OWNER', true) || '';
const GITHUB_REPO = tl.getInput('GITHUB_REPO', true) || '';
const GITHUB_ISSUE_NUMBER = tl.getInput('GITHUB_ISSUE_NUMBER', true) || '';
...
const readTerraformPlan = () => {
    try {
        const rawData = fs.readFileSync(TERRAFORM_PLAN_PATH, 'utf-8');
        return JSON.parse(rawData);
    }
    catch (error) {
        console.error(`Error reading or parsing the Terraform plan file: ${error.message}`);
        return null;
    }
};
exports.readTerraformPlan = readTerraformPlan;
// Function to determine the appropriate status icon based on precedence
const getStatusIcon = (adds, updates, deletes, imports) => {
    if (deletes > 0) {
        return '🛑';
    }
    else if (updates > 0) {
...
const getHigherPrecedenceStatus = (currentStatus, newStatus) => {
...
const summarizeTerraformPlan = (plan) => {
...
exports.summarizeTerraformPlan = summarizeTerraformPlan;
// Generate the summary for the environment as bullet points
const generateJobSummary = (adds, updates, deletes, imports, environment) => {
...
const calculateOverallStageStatus = (jobSummaries) => {
...
exports.calculateOverallStageStatus = calculateOverallStageStatus;
// Generate the entire section for the stage, including the headline and job summaries
const generateStageSection = (stageName, stageLink, jobSummaries) => {
    // Calculate the overall status of the stage based on job summaries
    const overallStatus = (0, exports.calculateOverallStageStatus)(jobSummaries);
...
const updateOrInsertSection = (existingCommentBody, newJobSummary, stageName, environment) => {
    ...
    const newSection = (0, exports.generateStageSection)(stageName, ADO_STAGE_LINK, jobSummaries); // Use ADO_STAGE_LINK here
